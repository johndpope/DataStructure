public struct MaxHeap<T: Comparable> {
    ...
    public mutating func pop() -> T {
        ...
        let _max_ = max(self.storage[0], self.storage[1])
        
        if _max_ == self.storage[0] {
    ...
    mutating func sink(_ index: Int) {
        ...
        default:
            let max = Swift.max(storage[vindex(lidx)],
                                storage[vindex(ridx)])
            
            if storage[vindex(index)] > max {
                return
            }
            
            if max == storage[vindex(lidx)] {
                swap_and_sink(index, lidx)
            } else {
                swap_and_sink(index, ridx)
            }
